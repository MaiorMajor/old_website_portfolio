...
<body style="font-family: 'Lexend Zetta'">
  <header>      <button><a href="https://web.fe.up.pt/~up201705355/Jorge+Marques/musica/home.php">Voltar ao site<a></button>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $db = new SQLite3('db_subscriber_musica.sqlite');

    // Verifica se o email existe na tabela
    $query = $db->prepare("SELECT * FROM subscriber_maior WHERE email = :email");
    $query->bindValue(':email', $email, SQLITE3_TEXT);
    $result = $query->execute();

    if ($row = $result->fetchArray()) {
      // Remove o utilizador da tabela
      $query = $db->prepare("DELETE FROM subscriber_maior WHERE email = :email");
      $query->bindValue(':email', $email, SQLITE3_TEXT);
      $query->execute();
